...
def _product_for_barcode(value: str) -> DetectedProduct | None:
    ...
    if not product.success:
        return None
    prices = [{"value": p.value, "currency": p.currency, "source": p.source, "kind": p.kind} for p in (product.prices or [])]
    price_note = None
    ...
    return DetectedProduct(product.name, product.brand, product.category, product.source, prices, price_note)


def _variants(image):
    """Create full-frame and tiled barcode/QR-friendly variants."""
    variants = [image]
    ...
    variants.append(cv2.adaptiveThreshold(enhanced, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 31, 8))

    # Tiny barcodes benefit more from enlarging a local region than enlarging
    # the entire image. Use overlapping 2x2 tiles so the code can be anywhere.
    if width >= 400 and height >= 400:
        tile_w = int(width * 0.60)
        tile_h = int(height * 0.60)
        step_x = int(width * 0.40)
        step_y = int(height * 0.40)
        for y in (0, step_y):
            for x in (0, step_x):
                crop = gray[min(y, height - tile_h):min(y, height - tile_h) + tile_h,
                            min(x, width - tile_w):min(x, width - tile_w) + tile_w]
                if crop.size:
                    tile = cv2.resize(crop, None, fx=2.0, fy=2.0, interpolation=cv2.INTER_CUBIC)
        ...
        if found:
            for value, code_type in zip(values, code_types):
                ...
                if value and value not in seen_values:
                    found_codes.append(DetectedCode(code_type or "BARCODE", value, _product_for_barcode(value)))
    ...
    try:
        ...
        if found and values is not None:
            for value in values:
                value = (value or "").strip()
                if value and value not in seen_values:
                    found_codes.append(DetectedCode("QR_CODE", value))
                    seen_values.add(value)
        else:
