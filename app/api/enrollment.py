...
from app.services.face_enrollment import FaceIntroduction, enroll_introduced_person
...
async def enroll_multi_face(
    ...
    if len(images) > MAX_ENROLLMENT_FRAMES:
        raise HTTPException(status_code=400, detail=f"Send no more than {MAX_ENROLLMENT_FRAMES} enrollment frames.")
    ...
    try:
        ...
        # Enrollment intentionally skips the normal strict frame-quality gate.
        # Natural movement is allowed; unusable individual frames are skipped
        # by face embedding extraction instead of failing the whole enrollment.
        result = enroll_introduced_person(
            image_paths=temp_paths,
            introduction=FaceIntroduction(name=clean_name, relationship=clean_relationship),
        )
        return {
            ...
            "good_quality_frames": result.accepted_frames,
        }
    finally:
